...
- `LOCAL`: NPL Engine running in docker running locally
- `NOUMENA_CLOUD`: NPL Engine running on NOUMENA CLOUD

## Combinations

1. Dev mode: `LOCAL` deployment target + `DEV_MODE` login
...
4. Cloud deployment (with ROPC authentication flow): `NOUMENA_CLOUD` deployment target + `OIDC` login
5. Cloud deployment (with Authorization Code authentication flow): `NOUMENA_CLOUD` deployment target + `KEYCLOAK` login
